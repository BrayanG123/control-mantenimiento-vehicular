...
class RegistrarMantenimientoPantalla extends StatefulWidget {
  ...
  final ProximoMantenimiento item;
...
class _RegistrarMantenimientoPantallaState
  ...
  Future<void> guardar() async {
    ...
    try {
      await DependenciasAplicacion.mantenimientoApi.registrarMantenimiento(
  ...
  Widget build(BuildContext context) {
    final titulo = obtenerTituloMantenimiento(widget.tipo);
    final km = _kmIngresado();
    ...
    final vencido = widget.item.estaVencido;
      ...
      body: ListView(
        ...
        children: [
          ...
          Semantics(
            button: true,
            enabled: !guardando,
            label: 'Fecha del servicio, ${formatearFecha(fecha)}',
          ...
          Column(
            ...
            children: [
              ...
              Container(
                ...
                padding: const EdgeInsets.symmetric(horizontal: 16),
                decoration: BoxDecoration(
                  color: hayErrorCosto ? const Color(0xFFFEF6F6) : Colors.white,
                  borderRadius: BorderRadius.circular(12),
                  border: Border.all(
                    color: hayErrorCosto
                        ? const Color(0xFFC93D3D)
              ...
              Row(
                children: [
                  Icon(
                    ...
                    size: 14,
                    color: hayErrorCosto ? const Color(0xFFC93D3D) : muted,
                  ),
                  const SizedBox(width: 6),
                  Expanded(
                    child: Text(
                      hayErrorCosto ? errorCosto! : 'Opcional. En bolivianos.',
                      style: TextStyle(
                        fontSize: 12,
                        color: hayErrorCosto ? const Color(0xFFC93D3D) : muted,
                      ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  'Km actual del vehiculo: ${formatearMiles(widget.kilometrajeActual)} km',
                  style: const TextStyle(fontSize: 12, color: muted),
                ),
                Text(
                  vencido
                      ? 'Vencido por ${formatearMiles(widget.item.kilometrosRestantes.abs())} km'
                      : 'Proximo a los ${formatearMiles(widget.item.proximoKilometraje)} km',
                  style: const TextStyle(fontSize: 12, color: muted),
                ),
              ],
            ),
